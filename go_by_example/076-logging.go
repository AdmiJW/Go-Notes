...
// with the `log` package for free-form output and the `slog` package for structured logging.
...
func Logging() {
	...
	// These are collected in `log.LstdFlags`. We can change its flags to emit time with microsecond precision, for example.
