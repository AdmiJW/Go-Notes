...
// Similar to the JSON examples, field tags contain directives for the encoder and decoder.
// Here we use some special features of the XML package: the `XMLName` field name dictates the name of
// the element representing this struct;
...
func XML() {
	...
	// The `parent>child>plant` field tag tells the encoder to nest all `plant` elements under `<parent><child>`...
	type Nesting struct {
