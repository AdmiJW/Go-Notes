...
// We'll be testing this simple implementation of an integer minimum. The code we're testing would
// be in a source file named something like `intutils.go`, and the test file for it would then be named `intutils_test.go`.
...
// Benchmark tests typically go in `_test.go` files and are named with a `Benchmark` prefix.
// The `testing` runner executes each benchmark function several times, increasing `b.N` on each run
...
// To run the tests, run `go test` from the package directory.
//    go test -v ./...

// To run the benchmarks, run `go test -bench`
//    go test -bench=./...
// All tests are run prior to benchmarks. The `bench` flag filters benchmark function names with a regexp.