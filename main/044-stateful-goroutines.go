...
// Our state will be owned by a single goroutine. This will guarantee that the data is never corrupted with concurrent access.
...
func StatefulGoroutines() {
	...
	// Each read requires constructing a `readOp`, sending it over the `reads` channel, and then
	...
	// It might be useful in certain cases though, for example when you have other channels involved or when
	// managing multiple such mutexes would be error-prone. You should use whichever approach feels most natural,
