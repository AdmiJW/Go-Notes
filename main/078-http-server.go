...
func HTTPServer() {
	// A fundamental concept in `net/http` servers is handlers. A handler is an object implementing
	// the `http.Handler` interface. A common way to write a handler is by using the `http.HandlerFunc` adapter
	...
	// This handler does something a little more sophisticaed by reading all the HTTP request headers
	...
	// Finally, we call the `http.ListenAndServe` with the port and a handler.
