...
func SpawningProcesses() {
	...
	// Next we'll look at a slightly more involved case where we pipe data to the external process on its `stdin`
	...
	// `date` doesn't have a `-x` flag so it will exit with an error message and a non-zero return code.
}